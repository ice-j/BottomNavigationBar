...
namespace BottomNavigationBar
{
    public class BottomBar : RelativeLayout, View.IOnClickListener, View.IOnLongClickListener
    {
        ...
        private ViewPropertyAnimator _translationAnimator;
        private static readonly IInterpolator INTERPOLATOR = new LinearInterpolator();
        ...
        /// <param name="savedInstanceState">a Bundle for restoring the state on configuration change.</param>
        /// <param name="accentColorRGBA">the color of the tab icons.</param>
        /// <returns>a BottomBar at the bottom of the screen.</returns>
        public static BottomBar Attach(View view, Bundle savedInstanceState, int accentColorRGBA)
        {
            BottomBar bottomBar = new BottomBar(view.Context, accentColorRGBA);
            bottomBar.OnRestoreInstanceState(savedInstanceState);
        ...
        public static BottomBar AttachShy(CoordinatorLayout coordinatorLayout, View userContentView, Bundle savedInstanceState)
        {
            ...
            bottomBar.ToughChildHood(coordinatorLayout.FitsSystemWindows);
            bottomBar.OnRestoreInstanceState(savedInstanceState);
        ...
        private void AnimateOffset(int offset, bool fast)
        {
            ...
            {
                _translationAnimator = this.Animate();
        ...
        public BottomBar(Context context, int accentColorRGBA)
            : base(context)
        {
            Init(context, null, 0, 0, accentColor: accentColorRGBA);
        }
        ...
        private void Init(Context context, IAttributeSet attrs, int defStyleAttr, int defStyleRes, bool colorsInitialized = false, int accentColor = -1)
        {
            _context = context;

            _darkBackgroundColor = new Color(MiscUtils.GetColor(Context, Resource.Color.bb_darkBackgroundColor));

            if (!colorsInitialized)
            {
                _whiteColor = new Color(MiscUtils.GetColor(Context, Resource.Color.white));
                if (accentColor == -1)
                    _primaryColor = new Color(MiscUtils.GetColor(_context, Resource.Attribute.colorPrimary));
                else
                    _primaryColor = new Color(accentColor);
            }

            _inActiveColor = new Color(MiscUtils.GetColor(Context, Resource.Color.bb_inActiveBottomBarItemColor));
        ...
        private void InitializeViews()
        {
            ...
            this.Elevation = MiscUtils.DpToPixel(_context, 8);
        ...
        private void UpdateItems(BottomBarItemBase[] bottomBarItems)
        {
            ...
            foreach (var bottomBarItemBase in listOfBottomBarItems)
            {
                ...
                var icon = (ImageView)bottomBarTab.FindViewById(Resource.Id.bb_bottom_bar_icon);
        ...
        private void DarkThemeMagic()
        {
            ...
            else
            {
                ...
                _tabletRightBorder.SetBackgroundColor(new Color(MiscUtils.GetColorCustom(_context, Resource.Color.bb_tabletRightBorderDark)));
            }
        }
        ...
        private void SelectTab(View tab, bool animate)
        {
            ...
            var icon = (ImageView)tab.FindViewById(Resource.Id.bb_bottom_bar_icon);
            ...
            if (animate)
            {
                var titleAnimator = title.Animate()
        ...
        private void UnselectTab(View tab, bool animate)
        {
            ...
            {
                if (title != null)
                    title.Alpha = _tabAlpha;

                icon.Alpha = _tabAlpha;
            }
