...
namespace BottomNavigationBar
{
	public class BottomBar : RelativeLayout, View.IOnClickListener, View.IOnLongClickListener
	{
		...
		private void Init(Context context, IAttributeSet attrs, int defStyleAttr, int defStyleRes, bool colorsInitialized = false)
		{
			...
			_darkBackgroundColor = new Color(Context.GetColor(Resource.Color.bb_darkBackgroundColor));

			if (!colorsInitialized)
			{
				_whiteColor = new Color(Context.GetColor(Resource.Color.white));
				_primaryColor = new Color(MiscUtils.GetColor(Context, Resource.Attribute.colorPrimary));
			}

			_inActiveColor = new Color(Context.GetColor(Resource.Color.bb_inActiveBottomBarItemColor));
		...
		private void DarkThemeMagic()
		{
			...
			else
			{
				...
				_tabletRightBorder.SetBackgroundColor(new Color(_context.GetColor(Resource.Color.bb_tabletRightBorderDark)));
			}
		}
